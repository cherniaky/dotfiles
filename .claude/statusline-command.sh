...
if [ -n "$used" ]; then
  ...
  ctx_str="${used_int}%"
  ctx_used=$(echo "$input" | jq -r '((.context_window.current_usage.cache_read_input_tokens // 0) + (.context_window.current_usage.cache_creation_input_tokens // 0) + (.context_window.current_usage.input_tokens // 0) + (.context_window.current_usage.output_tokens // 0)) | if . == 0 then empty else . end' 2>/dev/null)
...
if [ -n "$seven_d" ]; then
  [ "$line2_has_content" -eq 1 ] && printf "%b" "$SEP"
...
if [ -n "$ctx_str" ]; then
  [ "$line2_has_content" -eq 1 ] && printf "\033[90m | \033[0m"
