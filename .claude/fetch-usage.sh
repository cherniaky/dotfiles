...
if [ -z "$token" ]; then
  # Try macOS Keychain first (Claude Code 2.x stores creds here)
  KEYCHAIN_USER="${USER:-$(whoami)}"
  creds_json=$(security find-generic-password -s "Claude Code-credentials" -a "$KEYCHAIN_USER" -w 2>/dev/null)
